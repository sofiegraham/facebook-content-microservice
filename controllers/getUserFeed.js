...
const getUserFeed = (req, res, next) => {
  return db.knex.schema.raw(`WITH followers AS (SELECT follow_id FROM users_follows WHERE user_id = ${req.params.id}) SELECT * FROM posts WHERE (user_id IN (SELECT follow_id FROM followers) AND page_id = null) OR  (page_id IN (SELECT follow_id FROM followers));`)
    .then((data) => {
      req.feed = data.rows;
      next();
    }).catch((err) => {
